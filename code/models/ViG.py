...
#------------------------------------------------
@register_model
def Get_GCN(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            ...
            self.n_filters = 384  # number of channels of deep features
...
class GCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.pos_embed = nn.Parameter(torch.randn(1, channels, 8, 8))
...
if __name__ == '__main__':
    model = Get_GCN(num_classes=4)
    img = torch.zeros((2, 384, 8, 8))
    output = model(img)
    print(output.shape)