...
class Graph_VNet(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.down1 = Down_block(in_channels = self.channels, out_channels = self.channels, k=10, euclidean=True)
    ...
    def forward(self, x):
        ...
        x3 = x3.reshape(x3.shape[0], x3.shape[1]).contiguous()
        x3 = self.linear(x3)
        x3 = x3.reshape(x3.shape[0], x3.shape[1], 1, 1).contiguous()
