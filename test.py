...
class Options():
    def __init__(self):
        ...
        self.parser.add_argument("--threshold", default=0.7,help="a patch can be classified as normal if the prediciton is greater than threshold")
        self.parser.add_argument('--vit_path', type=str, default='./weight/finetune/16patch_encoder.pth')
        self.parser.add_argument('--vnet_path', type=str, default='./weight/finetune/16vnet.pth')
        self.parser.add_argument('--img_save_path', type=str, default='./snapshot/test_16/')
...
if __name__ == '__main__':
    ...
    for subject_id in opt.test_samples:
        ...
        for j in range(patch_prediction.shape[0]):
            for k in range(patch_prediction.shape[1]):
                if patch_prediction[j][k][0] < opt.threshold:   
                    patch_prediction[j][k][0] = 0
